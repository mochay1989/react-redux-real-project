...
export class ProductUI extends Component {
  ...
  clickProducts = (id) => {
    ...
    if (clickitem) {
      ...
      const cartIndex=cart.findIndex((cart)=>cart.id===clickitem.id);
      this.props.addToCart({ ...clickitem, quantity: 1, disc: 0 });
      
        this.props.changeItemToCart(clickitem[cartIndex].quantity++)
